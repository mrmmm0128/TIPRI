    ...
    try {
      ...
      if (open != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('サブスクリプションの決済リンクを取得できませんでした'),
              backgroundColor: Color(0xFFFCC400),
            ),
          );
        }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('サブスクリプションの決済リンクを取得できませんでした: $e'),
          backgroundColor: Color(0xFFFCC400),
        ),
      );
    } finally {
    ...
    if (agentId == null && code.isNotEmpty && !linked) {
      ...
      if (linkedInfo != null) {
        ...
        ScaffoldMessenger.of(scaffoldContext).showSnackBar(
          SnackBar(
            content: Text('代理店とリンクしました（code: $code）'),
            backgroundColor: Color(0xFFFCC400),
          ),
        );
      }
